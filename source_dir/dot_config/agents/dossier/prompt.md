...
Create source-backed, senior-level research dossiers that preserve mechanisms, tradeoffs, failure modes, disagreements, and a practical ramp-up path. This skill is for direct user-facing dossier work; delegated research can still use the `deep-researcher` agent/command.
